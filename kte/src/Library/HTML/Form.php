...
class Form
{
    ...
    public function formGroup(
        ...
        if ($ariaLabel && $legend) {
            $label = <<<html
                <p id="$ariaLabelId">{$ariaLabel}</p>;
            html;
        } else {
            $label .= '<p id="' . $ariaLabelId . '">' . accessibility()->srOnly($ariaLabel) . '</p>';
        }
        return <<<HTML
            <div {$id} role="group" class="form-group" {$this->ariaLabel($ariaLabelId)}>
                    {$label}
                    {$content}    
            </div>
        HTML;
    ...
    public function cancel(string $path, string $class, string $value): string
    {
        $path = url($path);
        return <<<HTML
            <button type="button" name="cancel" class="{$class}" onclick="window.location.href='{$path}'">{$value}</button>
        HTML;
    }

    public function button(string $class, string $value): string
    {
        return <<<HTML
    ...
    public function submitCancel(
        ...
        return <<<HTML
            {$this->submit($classSubmit, $valueSubmit)}
            {$this->cancel($path, $classCancel, $valueCancel)}
        HTML;
    }
    ...
    public function endFormWithSubmitCancel(
        ...
        return <<<HTML
            ...
            'Effacer le formulaire ou Valider',
            null,
            $this->submitCancel($classSubmit, $valueSubmit, $path, $classCancel, $valueCancel)
        )}
            {$this->endForm()}
        HTML;
    }
    ...
    public function endFormWithAllButtons(
        ...
        return <<<HTML
            ...
            null,
            $this->allButtons($classSubmit, $valueSubmit, $path, $classCancel, $valueCancel, $classReset, $valueReset)
        )}
            {$this->endForm()}
        HTML;
    }
